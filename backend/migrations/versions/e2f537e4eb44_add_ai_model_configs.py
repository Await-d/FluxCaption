...
def upgrade() -> None:
    ...
    for index_name, columns, unique in indexes_to_create:
        if not index_exists(inspector, "ai_model_configs", index_name):
            ...
            try:
                op.create_index(index_name, "ai_model_configs", columns, unique=unique)
    ...
    if not foreign_key_exists(inspector, "ai_model_configs", "fk_model_provider"):
        ...
        try:
            ...
            if table_exists(inspector, "ai_provider_configs"):
                ...
                print(
                    "Warning: ai_provider_configs table doesn't exist, skipping foreign key creation"
                )
        except Exception as e:
    ...
    existing_models = set()
    try:
        result = conn.execute(sa.text("SELECT provider_name, model_name FROM ai_model_configs"))
            ...
            if dialect == "postgresql":
                ...
                conn.execute(
                    sa.text("""
                        ...
                        VALUES
                        (:id, :provider, :model, :display, TRUE, :type,
                         :context, :max_out, :in_price, :out_price,
                         :desc, :tags, :default, 0, TRUE, 0,
                         0, 0, :now, :now)
