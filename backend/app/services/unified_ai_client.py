...
class UnifiedAIClient:
    ...
    def _update_model_usage(self, provider: str, model_name: str):
        ...
        try:
            ...
            stmt = sa.select(ModelRegistry).where(
                ModelRegistry.provider == provider, ModelRegistry.name == model_name
            )
            model_record = self.session.execute(stmt).scalar_one_or_none()
    ...
    async def list_models(self, provider_name: str | None = None) -> list:
        """
...
def get_unified_ai_client(session: Session | None = None) -> UnifiedAIClient:
    """
