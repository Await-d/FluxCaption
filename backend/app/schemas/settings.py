...
class SettingsResponse(BaseModel):
    ...
    default_subtitle_format: Literal["srt", "ass", "vtt"] = Field(
        description="Default subtitle format"
    )
    ...
    favorite_media_paths: list[str] = Field(
        default_factory=list, description="List of favorite local media directory paths"
    )
    ...
    db_vendor: Literal["postgres", "mysql", "sqlite", "mssql"] = Field(
...
class SettingsUpdateRequest(BaseModel):
    ...
    jellyfin_max_retries: int | None = Field(
        default=None, ge=0, le=10, description="Jellyfin max retries"
    )
    jellyfin_rate_limit_per_second: int | None = Field(
        default=None, ge=1, le=100, description="Jellyfin rate limit"
    )

    # Ollama Configuration
    ollama_base_url: str | None = Field(default=None, description="Ollama API base URL")
    ollama_timeout: int | None = Field(
        default=None, ge=10, le=600, description="Ollama timeout in seconds"
    )
    ...
    writeback_mode: Literal["upload", "sidecar"] | None = Field(
        default=None, description="Writeback mode"
    )
    ...
    translation_preserve_formatting: bool | None = Field(
        default=None, description="Preserve formatting"
    )

    # Model Configuration
    default_mt_model: str | None = Field(default=None, description="Default MT model")
    asr_engine: Literal["faster-whisper", "funasr"] | None = Field(
        default=None, description="ASR engine"
    )
    ...
    asr_device: Literal["cpu", "cuda", "auto"] | None = Field(
        default=None, description="ASR device"
    )
    ...
    # Resource Limits
    max_concurrent_scan_tasks: int | None = Field(
        default=None, ge=1, le=10, description="Max scan tasks"
    )
    max_concurrent_translate_tasks: int | None = Field(
        default=None, ge=1, le=20, description="Max translate tasks"
    )
    max_concurrent_asr_tasks: int | None = Field(
        default=None, ge=1, le=10, description="Max ASR tasks"
    )
    max_upload_size_mb: int | None = Field(
        default=None, ge=1, le=2000, description="Max upload size MB"
    )
    max_audio_duration_seconds: int | None = Field(
        default=None, ge=60, le=14400, description="Max audio duration"
    )

    # Feature Flags
    enable_auto_scan: bool | None = Field(default=None, description="Enable auto scan")
    enable_auto_pull_models: bool | None = Field(
        default=None, description="Enable auto pull models"
    )
    enable_sidecar_writeback: bool | None = Field(
    ...
    translate_task_timeout: int | None = Field(
        default=None, ge=60, le=7200, description="Translate timeout"
    )
    asr_task_timeout: int | None = Field(default=None, ge=300, le=14400, description="ASR timeout")

    # Local Media Configuration
    favorite_media_paths: list[str] | None = Field(
        default=None, description="Favorite local media paths"
    )
