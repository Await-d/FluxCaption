...
class MediaStream(BaseModel):
    ...
    codec: str | None = Field(None, alias="Codec", description="Codec name")
    language: str | None = Field(None, alias="Language", description="ISO 639-2 3-letter code")
    language_tag: str | None = Field(None, alias="LanguageTag", description="BCP-47 language tag")
    display_title: str | None = Field(
        None, alias="DisplayTitle", description="Human-readable title"
    )
    is_default: bool = Field(
        False, alias="IsDefault", description="Whether this is the default stream"
    )
    is_forced: bool = Field(False, alias="IsForced", description="Whether this is forced subtitle")
    is_external: bool = Field(
        False, alias="IsExternal", description="Whether this is external/sidecar"
    )
    is_text_subtitle_stream: bool = Field(
        False, alias="IsTextSubtitleStream", description="Whether this is text subtitle"
    )
    supports_external_stream: bool = Field(
        False, alias="SupportsExternalStream", description="Whether external streaming is supported"
    )
...
class MediaSource(BaseModel):
    ...
    path: str | None = Field(None, alias="Path", description="File path to media")
    container: str | None = Field(None, alias="Container", description="Container format")
    size: int | None = Field(None, alias="Size", description="File size in bytes")
    media_streams: list[MediaStream] = Field(
        default_factory=list, alias="MediaStreams", description="Available streams"
    )
...
class JellyfinLibrary(BaseModel):
    ...
    type: str | None = Field(
        None, alias="CollectionType", description="Type: movies, tvshows, music, etc."
    )
    item_count: int | None = Field(
        None, alias="ChildCount", description="Number of items in library"
    )
    image_tags: dict[str, str] | None = Field(
        None, alias="ImageTags", description="Image tags for different image types"
    )
    image_item_id: str | None = Field(
        None, description="ID of the item to use for image (library ID or first item ID)"
    )
    image_url: str | None = Field(None, description="Library primary image URL")
...
class JellyfinItem(BaseModel):
    ...
    path: str | None = Field(None, alias="Path", description="File system path")
    parent_id: str | None = Field(None, alias="ParentId", description="Parent library/series ID")
    run_time_ticks: int | None = Field(
        None, alias="RunTimeTicks", description="Duration in 100ns ticks"
    )
    media_sources: list[MediaSource] = Field(
        default_factory=list, alias="MediaSources", description="Available media sources"
    )
    container: str | None = Field(None, alias="Container", description="Container format")
    image_tags: dict[str, str] | None = Field(
        None, alias="ImageTags", description="Image tags for different image types"
    ...
    production_year: int | None = Field(
        None, alias="ProductionYear", description="Year of production"
    )
    community_rating: float | None = Field(
        None, alias="CommunityRating", description="Community rating"
    )
    official_rating: str | None = Field(
        None, alias="OfficialRating", description="Official content rating"
    )
    overview: str | None = Field(None, alias="Overview", description="Plot summary/description")
    genres: list[str] = Field(default_factory=list, alias="Genres", description="Genres")
    series_name: str | None = Field(
        None, alias="SeriesName", description="Series name (for episodes)"
    )
    season_name: str | None = Field(
        None, alias="SeasonName", description="Season name (for episodes)"
    )
    index_number: int | None = Field(None, alias="IndexNumber", description="Episode number")
    parent_index_number: int | None = Field(
        None, alias="ParentIndexNumber", description="Season number"
    )
...
class ScanLibraryRequest(BaseModel):
    ...
    library_id: str | None = Field(
        None, description="Specific library ID to scan (if None, scan all)"
    )
    required_langs: list[str] | None = Field(
        None, description="Required languages (overrides global config)"
    )
...
class ItemDetailResponse(BaseModel):
    ...
    missing_subtitle_langs: list[str] = Field(
        ..., description="Required subtitle languages that are missing"
    )
