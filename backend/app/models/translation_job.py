...
class TranslationJob(BaseModel):
    ...
    # Job status: queued | running | completed | failed | cancelled | paused
    ...
    current_phase: Mapped[str | None] = mapped_column(String(16), nullable=True)
    ...
    asr_output_path: Mapped[str | None] = mapped_column(String(1024), nullable=True)

    # Completed phases (JSON array: ["pull", "extract", "asr"])
    completed_phases: Mapped[str | None] = mapped_column(Text, nullable=True)

    # Translated target languages (JSON array: ["zh-CN", "en"])
    # Tracks which languages have been successfully translated
    completed_target_langs: Mapped[str | None] = mapped_column(Text, nullable=True)
