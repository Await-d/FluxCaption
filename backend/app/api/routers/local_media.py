...
from app.services.local_media_scanner import LocalMediaScanner, MediaFile
from app.workers.tasks import translate_subtitle_task, asr_then_translate_task
...
class ScanDirectoryRequest(BaseModel):
    ...
    required_langs: Optional[List[str]] = Field(None, description="必需的语言代码列表（如 ['zh-CN', 'en']）")
...
class CreateLocalJobRequest(BaseModel):
    ...
    source_lang: Optional[str] = Field(None, description="源语言（可选，用于已有字幕的翻译）")
...
async def scan_directory(
    ...
    try:
        ...
        logger.info(
            f"Scanned directory {request.directory}: "
        ...
        raise HTTPException(
            status_code=403,
            detail=f"Permission denied accessing directory: {str(e)}"
        )
...
async def get_directory_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get directory stats: {str(e)}"
        )
    ...
    try:
        ...
        for target_lang in request.target_langs:
            ...
            if request.source_lang:
                ...
                # 提交 Celery 任务
                translate_subtitle_task.apply_async(
                    args=[str(job.id)],
                    queue='translate'
                )
                ...
                # 提交 Celery 任务
                asr_then_translate_task.apply_async(
                    args=[str(job.id)],
                    queue='asr'
                )

        logger.info(
            f"Created job for local file {filepath.name}: "
            f"targets={request.target_langs}"
        )
...
async def create_batch_local_jobs(
    ...
    source_lang: Optional[str] = Query(None, description="源语言（可选）"),
    db: Session = Depends(get_db),
) -> List[JobResponse]:
    """
    批量创建本地媒体文件翻译任务
