...
async def get_system_config(
    ...
    stmt = (
        select(Setting)
        .where(Setting.is_editable == True)
    ...
    for setting in settings:
        ...
        if setting.key in CONFIG_CONSTRAINTS:
            setting_response.constraints = SettingConstraints(
                **CONFIG_CONSTRAINTS[setting.key]
            )
...
async def update_system_config(
    ...
    try:
        ...
        logger.error(
            f"Failed to publish config change event for '{key}': {e}",
            exc_info=True
        )
...
async def get_setting_change_history(
    ...
    return [
        SettingChangeHistoryResponse.model_validate(record)
